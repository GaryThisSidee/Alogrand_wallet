...
def algod_client():
    ...
    algod_token = "put ur own api key here"
    headers = {
...
def send_transaction(sender, quantity, receiver, note, sk):
    ...
    note = note.encode()
    try:
        unsigned_txn = PaymentTxn(sender, params, receiver, quantity, None,
                                  note)
...
def wait_for_confirmation(transaction_id, timeout):
    ...
    while current_round < start_round + timeout:
        try:
            pending_txn = algod_client().pending_transaction_info(
                transaction_id)
        except Exception as err:
            print(err)
            return
...
def create_asset(creator, asset_name, unit_name, total, decimals,
                 default_frozen, url, sk):
    """Creates an asset, returns the newly created asset ID"""
    params = algod_client().suggested_params()

    txn = AssetConfigTxn(sender=creator,
                         sp=params,
                         total=total,
...
def create_nft(creator, sk, metadataJSON, unit_name, asset_name, url):
